  ...
  async function procesarTexto(nombre: string, texto: string) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(tr("No es un fichero de entrega."))
      }
      if (!esSobre(bruto)) throw new Error(tr("No es una entrega de Hilo."))
      if (bruto.toma !== toma.id) throw new Error(tr("Es de otra toma."))
      ...
      setLineas(l => [{ nombre, texto: e instanceof Error ? tr(e.message) : 'Error', ok: false }, ...l])
    }
  }
    ...
    <>
      <div className="cabecera">
        <div>
          ...
          <h1>{tr("Recoger")}<br /><span className="light">{tr("las entregas")}</span></h1>
          <p className="lede">{tr("Descarga las entregas de la tarea (ficheros .txt, o el texto pegado si la tarea era de texto en línea) y súbelas aquí de golpe. Se abren con la clave privada de este dispositivo y se registran. Los repetidos se descartan.")}</p>
        </div>
      </div>
      {toma.estado !== 'abierta' && <p className="error">{tr("La toma está cerrada: no se registrará nada.")}</p>}

      <dl className="cifras">
        <div><dt>{tr("Han respondido")}</dt><dd>{participaciones?.length ?? 0}<small>de {alumnos.length}</small></dd></div>
        <div><dt>{tr("Registradas ahora")}</dt><dd>{lineas.filter(l => l.ok).length}</dd></div>
      </dl>

      <section className="sec">
        <div className="sec-head"><span className="sec-num">01</span><h2>{tr("Ficheros")}</h2></div>
      ...
      <section className="sec">
        <div className="sec-head"><span className="sec-num">02</span><h2>{tr("Pegar")}</h2></div>
        <form onSubmit={e => { e.preventDefault(); void procesarTexto('pegado', pegado.trim()); setPegado('') }}>
          ...
          <button type="submit" className="btn pequeno">{tr("Registrar")}</button>
        </form>
      </section>
    </>
  )
