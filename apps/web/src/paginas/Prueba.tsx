    ...
    void (async () => {
      try {
        ...
        const paquete = await empaquetarSesion({
          toma: toma.id, titulo: toma.titulo, idioma: toma.idioma, etapa: toma.etapa, situaciones: toma.situaciones, preguntas: toma.preguntas,
        ...
        setError(e instanceof Error ? tr(e.message) : 'No se pudo preparar la prueba.')
      }
    })()
  }, [toma, alumnos])
  ...
  return (
    <>
      ...
      <section className="sec">
        <div className="sec-head"><span className="sec-num">01</span><h2>{tr("Distribuir")}</h2></div>
        <label className="campo"><span>{tr("Enlace de la prueba")}</span><textarea readOnly value={url} style={{ minHeight: 90, maxWidth: 'none' }} /></label>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button type="button" className="btn secundario" disabled={!url} onClick={() => descargarTexto(`prueba-${toma.titulo}.html`.replace(/\s+/g, '_'), ficheroLanzador(url, `Mi equipo · ${toma.titulo}`), 'text/html')}>{tr("Descargar como fichero HTML")}</button>
        </div>
        <p className="aviso">{tr("El fichero HTML solo abre el enlace: sirve para colgarlo en una tarea de Moodle o en el aula virtual. La lista del grupo va en la parte del enlace que el navegador nunca envía a ningún servidor.")}</p>
      </section>

      <section className="sec">
        <div className="sec-head"><span className="sec-num">02</span><h2>{tr("Códigos de acceso")}</h2></div>
        <p>{tr("Cada alumno necesita su código. Es el mismo de la hoja de códigos de MiClase, si la usas. Entrégalo en mano: «Cartas para el alumnado» imprime una carta por alumno con su código, el enlace, un QR y las instrucciones de la tarea; «Etiquetas de códigos» imprime etiquetas recortables solo con nombre y código. Nunca en abierto junto al enlace.")}</p>
        <div className="tablewrap">
          <table><thead><tr><th>{tr("Alumno")}</th><th>{tr("Código")}</th><th>{tr("Entregado")}</th></tr></thead>
        ...
        <button type="button" className="btn secundario pequeno" onClick={() => print()}>{tr("Imprimir códigos")}</button>
      </section>

      <section className="sec">
        <div className="sec-head"><span className="sec-num">03</span><h2>{tr("Validez")}</h2></div>
        <div className="note" style={{ '--c': 'var(--m-social)' } as React.CSSProperties}><span className="tag">{tr("Misma sesión")}</span><p>{tr("Conviene que todo el grupo responda en la misma franja aunque cada uno esté en su casa: un sociograma respondido a lo largo de una semana mide otra cosa. Cierra la toma cuando hayas recogido las entregas.")}</p></div>
      </section>
    </>
  )
}
