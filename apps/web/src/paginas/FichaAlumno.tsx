...
export function FichaAlumno() {
  ...
  if (alumno === undefined) return <p className="mono">Cargando…</p>
  if (!alumno || !grupo || !alumnos) return <p>Este alumno no existe.</p>
  ...
  return (
    <>
      <div className="cabecera">
        ...
        <div className="acciones no-imprimir"><button type="button" className="btn secundario" onClick={() => print()}>Imprimir ficha</button></div>
      </div>

      <section className="sec">
        <div className="sec-head"><span className="sec-num">01</span><h2>Trayectoria</h2></div>
        {svg ? <div className="grafo" dangerouslySetInnerHTML={{ __html: svg }} /> : <p className="aviso">Con dos o más tomas aparece aquí la trayectoria, con los eventos del grupo sobre la misma línea.</p>}
        <div className="tablewrap">
          <table>
            <thead><tr><th>Toma</th><th className="num">Recibidas</th><th className="num">Emitidas</th><th className="num">Reciprocidad</th><th className="num">Ajuste</th><th>Posición</th><th>Tipo</th></tr></thead>
            <tbody>
              {filas.map(f => (
                <tr key={f.toma.id}>
      ...
      <section className="sec">
        <div className="sec-head"><span className="sec-num">03</span><h2>Notas del docente</h2></div>
        {notas && notas.length > 0 && (
          <ul className="rules">
            {notas.map(nt => (
              <li key={nt.id}><span className="dash">—</span><span className="crece">{nt.texto}</span><span className="meta">{nt.fecha}</span><button type="button" className="enlace no-imprimir" onClick={() => { if (confirm('¿Borrar esta nota?')) void borrarNota(nt.id) }}>borrar</button></li>
            ))}
          </ul>
        )}
        <form className="fila no-imprimir" onSubmit={async e => { e.preventDefault(); if (!texto.trim()) return; await anadirNota(alumno.id, fecha, texto); setTexto('') }}>
          ...
          <label className="campo" style={{ flex: '1 1 300px' }}><span>Observación</span><input type="text" value={texto} onChange={e => setTexto(e.target.value)} /></label>
...
function Vinculos({ alumno, toma, analisis, nombre }: { alumno: Alumno; toma: Toma; analisis: ReturnType<typeof analizar>; nombre: Map<string, string> }) {
  ...
  return (
    <section className="sec">
      ...
      <dl className="cifras">
        <div><dt>Elige a</dt><dd style={{ fontSize: 16 }}>{eligeA.length ? eligeA.map(n).join(', ') : '—'}</dd></div>
        <div><dt>Le eligen</dt><dd style={{ fontSize: 16 }}>{leEligen.length ? leEligen.map(n).join(', ') : '—'}</dd></div>
        <div><dt>Recíprocos</dt><dd style={{ fontSize: 16 }}>{reciprocos.length ? reciprocos.map(n).join(', ') : '—'}</dd></div>
      </dl>
      <p className="aviso">Situaciones de preferencia: {toma.situaciones.filter(s => s !== 'espejo').map(s => ETIQUETA_SITUACION[s]).join(', ')}.</p>
    </section>
  )
}
