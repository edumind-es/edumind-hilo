...
describe('informe de grupo', () => {
  it('es un documento autocontenido, con los datos escapados y sin orígenes externos', async () => {
    const alumnos = ['a', 'b', 'c'].map((id, i) => ({ id, ...base, grupo_id: 'g', nombre: i === 0 ? 'Ana <script>' : `N${i}`, codigo: 'AB3DE', neae: false }))
    const toma = { id: 't', ...base, grupo_id: 'g', titulo: 'Septiembre', etapa: 'primaria' as const, idioma: 'es' as const, situaciones: ['equipo' as const, 'espejo' as const], preguntas: [], max_elecciones: 3, negativas: false, estado: 'cerrada' as const, inicio: ahora, fin: ahora }
    ...
    const html = await documentoInforme({ grupo, toma, alumnos, analisis, nombres: new Map(alumnos.map(a => [a.id, a.nombre])), respondieron: 2 })
