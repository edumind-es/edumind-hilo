...
export function Toma() {
  ...
  if (!toma || !grupo || !alumnos || !analisis) return <p>{tr("Esta toma no existe.")} <Link to="/">{tr("Volver")}</Link>.</p>
    ...
    <>
      <div className="cabecera">
        ...
        <div className="acciones no-imprimir">
          {toma.estado === 'abierta' ? (
            <>
              <Link className="btn" to={`/toma/${toma.id}/responder`}>{tr("Responder en este dispositivo")}</Link>
              <Link className="btn secundario" to={`/toma/${toma.id}/sesion`}>{tr("Sesión con tablets")}</Link>
              <Link className="btn secundario" to={`/toma/${toma.id}/escanear`}>{tr("Leer con la cámara")}</Link>
              <Link className="btn secundario" to={`/toma/${toma.id}/hojas`}>{tr("Hojas de marcas")}</Link>
              <Link className="btn secundario" to={`/toma/${toma.id}/hojas-grupo`}>{tr("Hoja de grupo")}</Link>
              <Link className="btn secundario" to={`/toma/${toma.id}/transcribir`}>{tr("Transcribir")}</Link>
              <Link className="btn secundario" to={`/toma/${toma.id}/prueba`}>{tr("Prueba con código")}</Link>
              <Link className="btn secundario" to={`/toma/${toma.id}/etiquetas`}>{tr("Etiquetas de códigos")}</Link>
              <button type="button" className="btn secundario" onClick={() => { if (confirm(tr("Cerrar la toma. Ya no se podrán añadir respuestas. ¿Seguir?"))) void cerrarToma(toma.id) }}>{tr("Cerrar la toma")}</button>
            </>
          ) : <span className="stamp cerrada">{tr("Cerrada")}</span>}
          <Link className="btn secundario" to={`/toma/${toma.id}/informe`}>{tr("Informe")}</Link>
        </div>
      </div>
      ...
      <section className="sec">
        <div className="sec-head"><span className="sec-num">01</span><h2>{tr("Lista de atención")}</h2></div>
        {atencion.length === 0 ? <p>{tr("Nadie sin elecciones ni sin respuesta.")}</p> : (
          <ul className="rules">
            {atencion.map((x, i) => <li key={i}><span className="dash">—</span><span className="crece">{n(x.alumno_id)}</span><span className="meta">{tr(x.motivo)}</span></li>)}
          </ul>
        )}
        <p className="aviso">{tr("Es una lista de trabajo del tutor, no un semáforo. Los datos dicen quién está aislado hoy; no dicen por qué ni qué hacer.")}</p>
      </section>
      ...
      <section className="sec">
        <div className="sec-head"><span className="sec-num">03</span><h2>{tr("Índices por alumno")}</h2></div>
              ...
              {alumnos.map(a => {
                ...
                return (
                  <tr key={a.id}>
                    ...
                    <td>{tr(x.posicion)}</td>
                    {toma.negativas && <td>{x.tipo ? tr(x.tipo) : '—'}</td>}
                  </tr>
                )
              })}
            </tbody>
        ...
        <p className="aviso">{tr("Reciprocidad: de los que elige, cuántos le eligen. Ajuste: de quienes cree que le eligen (Espejo), cuántos le eligen de verdad. Posición: elecciones recibidas tipificadas dentro del grupo.")}</p>
      </section>

      <section className="sec">
        <div className="sec-head"><span className="sec-num">04</span><h2>{tr("Matriz sociométrica")}</h2></div>
        <Matriz alumnos={alumnos} analisis={analisis} situaciones={preferencia} negativas={toma.negativas} />
        <p className="aviso">{tr("Filas: quién elige. Columnas: a quién. La cifra es en cuántas situaciones le elige; «−» marca una negativa. La última columna suma lo recibido.")}</p>
      </section>

      <section className="sec">
        <div className="sec-head"><span className="sec-num">05</span><h2>{tr("Subgrupos y puentes")}</h2></div>
        {analisis.subgrupos.length === 0 ? <p>{tr("Todavía no hay parejas recíprocas.")}</p> : (
          <ul className="rules">
  ...
  return (
    <div className="tablewrap">
      <table className="matriz">
        <thead>
          <tr>
            <th className="fila">{tr("elige →")}</th>
            {alumnos.map(a => <th key={a.id} className="col">{a.nombre}</th>)}
            <th className="col">{tr("Recibidas")}</th>
          </tr>
        </thead>
        <tbody>
          {alumnos.map(a => {
